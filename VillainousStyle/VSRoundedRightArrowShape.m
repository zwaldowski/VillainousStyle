...
#import <VillainousStyle/VSRoundedRightArrowShape.h>

@implementation VSRoundedRightArrowShape
...
+ (VSRoundedRightArrowShape*)shapeWithRadius:(CGFloat)radius {
	VSRoundedRightArrowShape* shape = [[[VSRoundedRightArrowShape alloc] init] autorelease];
	shape.radius = radius;
	return shape;
}
...
- (void)addToPath:(CGRect)rect {
	...
	CGFloat radius = VSRadius(_radius);
...
- (UIEdgeInsets)insetsForSize:(CGSize)size {
	...
	return UIEdgeInsetsMake(0, 0, 0, floor(VSRadius(_radius)));
	//  CGFloat fh = size.height/3;
	//  return UIEdgeInsetsMake(floor(VSRadius(_radius)), floor(VSRadius(_radius)),
	//                          floor(VSRadius(_radius)), floor(VSRadius(_radius))*2);
}
