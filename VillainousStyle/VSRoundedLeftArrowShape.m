...
#import <VillainousStyle/VSRoundedLeftArrowShape.h>

@implementation VSRoundedLeftArrowShape
...
+ (VSRoundedLeftArrowShape*)shapeWithRadius:(CGFloat)radius {
	VSRoundedLeftArrowShape* shape = [[[VSRoundedLeftArrowShape alloc] init] autorelease];
	shape.radius = radius;
	return shape;
}
...
- (UIEdgeInsets)insetsForSize:(CGSize)size {
	...
	return UIEdgeInsetsMake(0, floor(VSRadius(_radius)), 0, 0);
	//  CGFloat fh = size.height/3;
	//  return UIEdgeInsetsMake(floor(VSRadius(_radius)), floor(VSRadius(_radius))*2,
	//                          floor(VSRadius(_radius)), floor(VSRadius(_radius)));
}
